...
def cell(config: dict) -> dict:
    ...
    source = [
        ...
        "        return str(v)\n",
        "    limits_rows = ''.join(\n",
        "        f\"<tr><td style='padding:3px 8px 3px 0;color:#94a3b8;text-transform:capitalize'>{k.replace('_', ' ')}</td>\"",
        "        f\"<td style='padding:3px 0;color:#e2e8f0'>{_fmt(v)}</td></tr>\"\n",
        "        for k, v in limits.items()\n",
        "    )\n",
        "    limits_html = (f\"<table style='border-collapse:collapse;margin-top:6px'>{limits_rows}</table>\"\n",
        "        if limits_rows else '')\n",
        "    sub_html = (f\"<b>{plan_label}</b><br>\"",
        "        f\"Status: {d.get('status', 'active')}<br>\"",
        "        f\"Monthly limit: ${d.get('monthly_limit_usd', 0):.2f}<br>\"",
        "        (f\"Top-up balance: ${d.get('topup_balance_usd', 0):.2f}<br>\" if d.get('topup_balance_usd') else '')",
        f"        + f\"<br><b>Plan limits</b>{limits_html}<br><a href='https://{domain}/pricing' style='color:#60a5fa'>Manage plan</a>\")\n",
        "else:\n",
